...
本文只提供 **Cloudflare 网页控制台** 部署教程。

> 一键部署会打开 Cloudflare 部署流程；部署前仍需要创建自己的 D1 数据库，并在控制台配置 `ADMIN_USERNAME`、`ADMIN_PASSWORD`、`SESSION_SECRET` 等 secrets。
...
然后把 `wrangler.jsonc` 中的 `database_id` 改成你自己的 ID，并提交到你自己的仓库：
...
检查 Worker 项目 **Settings** 中的 Secrets 是否配置完整：
...
- D1 binding 名是否为 `DB`
- `startnest-db` 是否已经在控制台执行全部 migration
...
- 首次部署前，请在 Cloudflare 控制台的 Worker **Settings** 中配置 `ADMIN_USERNAME`、`ADMIN_PASSWORD` 和 `SESSION_SECRET`。
- 数据库结构变更必须新增 migration，并在控制台按顺序执行新增 SQL，不要修改已发布 migration。
