...
function LinkPreview({ draft }: { draft: LinkDraft }) {
  ...
  const iconFrameClassName = `${iconClassName} flex items-center justify-center overflow-hidden rounded-xl bg-muted/70`
  ...
  const plainTextClassName = `${iconClassName} flex items-center justify-center font-bold text-primary ${iconOnly ? 'text-[1.4rem]' : 'text-sm'}`
  const framedTextClassName = `${iconFrameClassName} font-bold text-primary ${iconOnly ? 'text-[1.4rem]' : 'text-sm'}`
  ...
  const visual =
    draft.iconMode === 'image' && draft.iconImageUrl ? (
      ...
      <div className={iconFrameClassName}>
        <AppIcon name={draft.icon} className={`${iconOnly ? 'h-[70%] w-[70%]' : 'h-[60%] w-[60%]'} text-primary`} />
      </div>
    ) : draft.iconMode === 'text' ? (
  ...
  return (
    <div className="flex justify-center border-b pb-6">
      <Card
        style={cardStyle}
        className={`overflow-hidden text-foreground ${iconOnly ? 'flex h-[4.8rem] w-[4.8rem] flex-col items-center justify-center p-[5%]' : 'grid h-[4.8rem] w-[15rem] grid-cols-[4.25rem_minmax(0,1fr)] items-center gap-0 px-2.5 py-2'}`}
      >
        {iconOnly ? (
